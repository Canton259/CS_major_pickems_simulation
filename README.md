...
- Adds Bayesian map-strength shrinkage, seed-reproducible softmax veto, and a historical backtest/fitting CLI / 新增地图强度贝叶斯收缩、可复现 softmax veto，以及历史回测/拟合 CLI
...
- Historical Backtest / 历史回测
  - Playwright-based HLTV fetch cache under `.cache/hltv_history/` / 基于 Playwright 抓取并缓存 HLTV 历史页面
  - Match-level log loss evaluation and coarse-to-fine parameter fitting / 支持比赛级 log loss 评估与参数拟合
...
Playwright is only required for `backtest.py fetch`; local simulation, fixture backtests, and greedy parsing do not need browser access.
...
6. Historical Backtest and Fitting / 历史回测与参数拟合:
```bash
python backtest.py fetch --months 6
python backtest.py evaluate --history .cache/hltv_history
python backtest.py fit --history .cache/hltv_history --output fitted_params.json
python backtest.py apply --config major_stage.json --params fitted_params.json --output major_stage_fitted.json
```

`fetch` uses Playwright and caches raw HLTV pages under `.cache/hltv_history/`. The cache and `fitted_params.json` are ignored by git by default.
`fit` uses 90-day training / 30-day validation rolling windows when the dated history spans enough time; small fixtures fall back to all-match evaluation.
...
- `model_params.map_adjustment_weight`: Strength of map adjustment on top of base rating probability (Default: 0.2) / 地图强度对基础胜率的修正权重（默认: 0.2）
- `model_params.veto_temperature`: Softmax veto temperature. `0` restores deterministic map selection (Default: 0.15) / softmax veto 温度；`0` 恢复确定性选图（默认: 0.15）
- `model_params.bayes_prior_maps`: Bayesian prior map count for shrinking map WR before strength calculation (Default: 5.0) / 地图 WR 贝叶斯收缩的先验地图数（默认: 5.0）
...
- Historical fitting optimizes match-level log loss over cached history using a deterministic coarse-to-fine grid search / 历史拟合使用缓存历史数据上的比赛级 log loss，并通过确定性 coarse-to-fine grid search 寻优
...
- Veto choices use an 80/20 score: 80% HLTV historical pick/ban tendency and 20% relative map-strength advantage. `veto_temperature > 0` samples by softmax; the same simulation seed remains reproducible / veto 使用 80% HLTV 历史 pick/ban 倾向 + 20% 地图相对优势评分；`veto_temperature > 0` 时按 softmax 随机化，同 seed 仍可复现。
...
- `strength` is recalculated with Bayesian-shrunk WR: `shrunk_wr = (wins + 0.5 * bayes_prior_maps) / (maps_played + bayes_prior_maps)`, then passed through the user formula and clamped to `[0, 1]` / `strength` 先对 WR 做贝叶斯收缩，再进入用户公式并裁剪到 `[0, 1]`。
